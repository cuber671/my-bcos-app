...
import com.fisco.app.dto.EnterpriseWithUsersDTO;
import com.fisco.app.dto.UserWithEnterpriseDTO;
...
public class EnterpriseUserRelationController {
    ...
    public Result<?> getCurrentUserWithEnterprise(Authentication authentication) {
        ...
        try {
            if ("ENTERPRISE".equals(loginType) || "ADMIN".equals(loginType)) {
                ...
                com.fisco.app.dto.EnterpriseWithUsersDTO result =
    ...
    public Result<java.util.List<com.fisco.app.entity.User>> getActiveUsersByEnterprise(
            @ApiParam(value = "企业ID", required = true) @PathVariable @NonNull String enterpriseId) {
        log.info("获取企业活跃用户: enterpriseId={}", enterpriseId);
        java.util.List<com.fisco.app.entity.User> users = userService.getActiveUsersByEnterpriseId(enterpriseId);
