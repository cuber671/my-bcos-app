...
import com.fisco.app.entity.BillSettlement;
...
public interface BillSettlementRepository extends JpaRepository<BillSettlement, String> {
     ...
     * 统计结算成功率
     */
    @Query("SELECT CASE WHEN COUNT(s) > 0 THEN (CAST(SUM(CASE WHEN s.settlementStatus = 'COMPLETED' THEN 1 ELSE 0 END) AS FLOAT) / COUNT(s)) * 100 ELSE 0 END FROM BillSettlement s WHERE s.initiatorId = :initiatorId")
