...
import com.fisco.app.entity.BillRecourse;
...
public interface BillRecourseRepository extends JpaRepository<BillRecourse, String> {
     ...
     * 统计追索成功率
     */
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN (CAST(SUM(CASE WHEN r.recourseStatus = 'COMPLETED' THEN 1 ELSE 0 END) AS FLOAT) / COUNT(r)) * 100 ELSE 0 END FROM BillRecourse r WHERE r.initiatorId = :initiatorId")
