...
public interface ReceiptCancelApplicationRepository extends JpaRepository<ReceiptCancelApplication, String> {
     ...
     * 统计仓储企业的待审核申请数量
     */
    @Query("SELECT COUNT(a) FROM ReceiptCancelApplication a JOIN electronic_warehouse_receipt r ON a.receipt_id = r.id WHERE r.warehouse_id = :warehouseId AND a.requestStatus = 'PENDING'")
