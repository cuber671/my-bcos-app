...
public interface ReceiptSplitApplicationRepository extends JpaRepository<ReceiptSplitApplication, String> {
     ...
     * 检查是否存在待审核的拆分申请
     */
    @Query("SELECT COUNT(a) > 0 FROM ReceiptSplitApplication a WHERE a.parent_receiptId = :parentReceiptId AND a.requestStatus = 'PENDING'")
    boolean existsPendingSplitApplication(@Param("parentReceiptId") String parentReceiptId);
     ...
     * 统计仓储企业的待审核申请数量
     */
    @Query("SELECT COUNT(a) FROM ReceiptSplitApplication a JOIN electronic_warehouse_receipt r ON a.parent_receipt_id = r.id WHERE r.warehouse_id = :warehouseId AND a.requestStatus = 'PENDING'")
