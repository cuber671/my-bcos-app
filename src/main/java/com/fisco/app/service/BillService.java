...
import com.fisco.app.dto.DiscountBillRequest;
import com.fisco.app.dto.DiscountBillResponse;
import com.fisco.app.dto.EndorseBillRequest;
import com.fisco.app.dto.EndorsementResponse;
import com.fisco.app.dto.IssueBillRequest;
import com.fisco.app.dto.RepayBillRequest;
import com.fisco.app.dto.RepayBillResponse;
import com.fisco.app.entity.Bill;
import com.fisco.app.entity.DiscountRecord;
import com.fisco.app.entity.Endorsement;
import com.fisco.app.entity.RepaymentRecord;
import com.fisco.app.exception.BlockchainIntegrationException;
import com.fisco.app.repository.BillRepository;
import com.fisco.app.repository.DiscountRecordRepository;
import com.fisco.app.repository.EndorsementRepository;
import com.fisco.app.repository.RepaymentRecordRepository;
...
public class BillService {
    ...
    public void acceptBill(@NonNull String billId) {
        ...
        try {
            ...
            try {
                ...
                bill.setBillStatus(Bill.BillStatus.NORMAL);
    ...
    public EndorsementResponse endorseBill(@NonNull String billId, EndorseBillRequest request, String endorserAddress) {
        ...
        if (bill.getBillStatus() != Bill.BillStatus.ISSUED &&
            bill.getBillStatus() != Bill.BillStatus.NORMAL &&
        ...
        try {
            ...
            String txHash = contractService.endorseBillOnChain(billId, request.getEndorseeAddress(), endorsementTypeStr);
     ...
     */
    @Transactional
    public Bill cancelBill(@NonNull String billId, com.fisco.app.dto.CancelBillRequest request, String operatorAddress) {
        log.info("==================== 票据作废开始 ====================");
     ...
     */
    @Transactional
    public Bill freezeBill(@NonNull String billId, com.fisco.app.dto.FreezeBillRequest request, String operatorAddress) {
        log.info("==================== 票据冻结开始 ====================");
        log.info("票据ID: {}, 操作人: {}", billId, operatorAddress);
     ...
     */
    @Transactional
    public Bill unfreezeBill(@NonNull String billId, com.fisco.app.dto.UnfreezeBillRequest request, String operatorAddress) {
        log.info("==================== 票据解冻开始 ====================");
        log.info("票据ID: {}, 操作人: {}", billId, operatorAddress);
    ...
    private final com.fisco.app.repository.BillFinanceApplicationRepository financeApplicationRepository;

    /**
     * 票据融资申请
     */
    @Transactional(rollbackFor = Exception.class)
    public com.fisco.app.dto.FinanceApplicationResponse applyFinance(@NonNull String billId,
                                                                       com.fisco.app.dto.FinanceBillRequest request,
        ...
        try {
            ...
            // 步骤2: 创建融资申请
            log.debug("创建融资申请");
            com.fisco.app.entity.BillFinanceApplication application = new com.fisco.app.entity.BillFinanceApplication();
            ...
            com.fisco.app.entity.BillFinanceApplication savedApplication = financeApplicationRepository.save(application);
            log.info("✓ 融资申请已创建: applicationId={}", savedApplication.getId());

            // 步骤3: 构建响应
            com.fisco.app.dto.FinanceApplicationResponse response = buildFinanceResponse(savedApplication, bill);
            log.info("✓✓✓ 票据融资申请创建成功");
    ...
    public com.fisco.app.dto.FinanceApplicationResponse approveFinance(com.fisco.app.dto.ApproveFinanceRequest request,
        ...
        try {
            ...
            com.fisco.app.entity.BillFinanceApplication application = financeApplicationRepository.findById(applicationId)
            ...
            if (request.getApprovalResult() == com.fisco.app.dto.ApproveFinanceRequest.ApprovalResult.APPROVED) {
                // 审核通过
                log.debug("审核通过");
            ...
            com.fisco.app.dto.FinanceApplicationResponse response = buildFinanceResponse(application, bill);
     ...
     * 查询待审核融资申请
     */
    public java.util.List<com.fisco.app.dto.FinanceApplicationResponse> getPendingFinanceApplications(String institutionId) {
        log.info("查询待审核融资申请: institutionId={}", institutionId);

        java.util.List<com.fisco.app.entity.BillFinanceApplication> applications;
    ...
    @Transactional(rollbackFor = Exception.class)
    @SuppressWarnings("null")
    public com.fisco.app.dto.FinanceApplicationResponse repayFinance(@NonNull String applicationId,
                                                                     com.fisco.app.dto.RepayFinanceRequest request,
        ...
        try {
            ...
            if (request.getRepayType() == com.fisco.app.dto.RepayFinanceRequest.RepayType.FULL) {
                // 全额还款：本金 + 全部利息
     ...
     * 构建融资申请响应对象
     */
    private com.fisco.app.dto.FinanceApplicationResponse buildFinanceResponse(
            com.fisco.app.entity.BillFinanceApplication application,
            @org.springframework.lang.Nullable Bill bill) {
        com.fisco.app.dto.FinanceApplicationResponse response = new com.fisco.app.dto.FinanceApplicationResponse();
